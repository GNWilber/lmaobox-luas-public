...
if not success then
    print("[wilcrit ERROR] wilgui.lua missing or corrupted! Error: " .. tostring(wilgui))
    return
end
...
if success and wilgui then
    -- Remove ONLY wilcrit's menu on reload, protecting wilbind's menu
...
callbacks.Register("Draw", "wilcrit_Draw", function()
    ...
    if requiredDamage < 0 then requiredDamage = 0 end

    local data = {}
    if critTokenBucket < critCost then table.insert(data, "Shots needed for crit: " .. shotsNeededForTokens) end
    if requiredDamage > 0 then table.insert(data, "Damage needed for crit: " .. requiredDamage) end
...
callbacks.Register("Unload", "wilcrit_Unload", function()
    if success and wilgui and menu then wilgui.RemoveMenu(menu) end
end)